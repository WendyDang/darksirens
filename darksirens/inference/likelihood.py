...
logPriorUniverse = universe_model_parser(universe_model='spectral_sirens_fast')

@partial(jax.jit, static_argnames=['nEvents', 'Ndraw', 'nsamp', 'pop_model'])
def darksiren_log_likelihood(cosmo_params, survey_params, pop_params,
    ...
    log_det_weights += - jnp.log(ddL_of_z(zsels,dLsels,H0,Om0)) - jnp.log(p_draw) - 2*jnp.log1p(zsels) + logPriorUniverse(zsels,selsamples_ind,H0,Om0,n0,z1,z50,delta,gamma)
    ...
    log_weights += - jnp.log(ddL_of_z(z,dL,H0,Om0)) - jnp.log(p_pe) - 2*jnp.log1p(z) + logPriorUniverse(z,samples_ind,H0,Om0,n0,z1,z50,delta,gamma)
